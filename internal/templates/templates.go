...
func New{{ .Code }}Error({{ range .MetaData }}{{ .Name }} {{ .DataType }}, {{ end }}{{ if .IncludeMap }}fields map[string]interface{}, {{ end }}includeStack bool) errors.RichError {
	msg := "{{ .Message }}"
	err := errors.NewRichError(ErrCode{{ .Code }}, msg){{ if .IncludeMap }}.WithMetaData(fields){{ end }}{{ range .MetaData }}{{ if eq .DataType "error" }}.AddError({{ .Name }}){{ else }}.AddMetaData("{{ .Name }}", {{ .Name }}){{ end }}{{ end }}
	if includeStack {
		err = err.WithStack(1)
	}
	return err
